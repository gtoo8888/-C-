...
void StopwatchClockWdg::slotBtnDownload()
{
    ...
    for(int i = 0;i < nowCout;i++){
        QString outStrLine = QString("%1,%2,%3\n").arg(vTimeCount[i].index,2,10,QLatin1Char('0')).arg(vTimeCount[i].intervalTimeStr).
...
void StopwatchClockWdg::slotBtnTimeCount()
{
    ...
    qDebug() << QString("idx:%1,interval time:%2,total time:%3").arg(nowClockCount).arg(intervalTimeStr).arg(totalTimeStr);

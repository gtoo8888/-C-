...
int XVPlayer::initSws() {
    mvSwsOutSpec.widght = mvDecodeCtx->width;
    mvSwsOutSpec.height = mvDecodeCtx->width;
    mvSwsOutSpec.pixFmt = AV_PIX_FMT_RGB24;

    // 初始化视频像素格式转换上下文
    mvSwsCtx = sws_getContext(mvDecodeCtx->width, mvDecodeCtx->height, mvDecodeCtx->pix_fmt, mvSwsOutSpec.widght,
                              mvSwsOutSpec.height, mvSwsOutSpec.pixFmt, SWS_BILINEAR, nullptr, nullptr, nullptr);
    ...
    int ret = av_image_alloc(mvSwsOutFrame->data, mvSwsOutFrame->linesize, mvSwsOutSpec.widght, mvSwsOutSpec.height,
                             mvSwsOutSpec.pixFmt,
                             1);  // 表示对齐
    RET(av_image_alloc);
...
void XVPlayer::addVideoPkt(AVPacket &pkt) {
    mvMutex->lock();
...
void XVPlayer::clearVideoPktList() {
    mvMutex->lock();
...
void XVPlayer::decodeVideo() {
    while (true) {
        ...
        while (true) {
            ...
            sws_scale(mvSwsCtx, mvSwsInFrame->data, mvSwsInFrame->linesize, 0, mvDecodeCtx->height, mvSwsOutFrame->data,

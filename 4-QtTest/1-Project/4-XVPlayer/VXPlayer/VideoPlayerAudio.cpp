...
int XVPlayer::initSwr() {
    ...
    // 声道数*每个样本的大小
    maSwrOutSpec.bytesPerSampleFrame = maSwrOutSpec.chs * av_get_bytes_per_sample(maSwrOutSpec.sampleFmt);

    maSwrCtx = swr_alloc_set_opts(nullptr,
                                  // 输出参数
                                  maSwrOutSpec.chLayout, maSwrOutSpec.sampleFmt, maSwrOutSpec.sampleRate,
                                  // 输入参数
                                  maSwrInSpec.chLayout, maSwrInSpec.sampleFmt, maSwrInSpec.sampleRate, 0, nullptr);
    if (!maSwrCtx) {
    ...
    ret = av_samples_alloc(maSwrOutFrame->data, maSwrOutFrame->linesize, maSwrOutSpec.chs, 4096, maSwrOutSpec.sampleFmt,
                           1);
...
uint64_t count = 0;
void XVPlayer::sdlAudioCallbackFunc(void *userdata, Uint8 *stream, int len) {
    //    qDebug() << "count" << count++ << "len" << len;
    XVPlayer *player = (XVPlayer *)userdata;
    player->sdlAudioCallback(stream, len);
}

int XVPlayer::initSDL() {
    // 之后做重采样的处理
    // 音频参数
    SDL_AudioSpec spec;
    // 采样率
...
void XVPlayer::sdlAudioCallback(Uint8 *stream, int len) {
    SDL_memset(stream, 0, len);
    // len:SDL音频缓冲区剩余的大小(还未填充的大小)
    while (len > 0) {
        if (mState == Stopped) break;
...
int XVPlayer::decodeAudio() {
    ...
    // 重采样输出的样本
    int outSamples =
        av_rescale_rnd(maSwrOutSpec.sampleRate, maSwrInFrame->nb_samples, maSwrInSpec.sampleRate, AV_ROUND_UP);
    ...
    ret = swr_convert(maSwrCtx,
                      ...
                      outSamples, (const uint8_t **)maSwrInFrame->data, maSwrInFrame->nb_samples);
    RET(swr_convert);
...
void XVPlayer::addAudioPkt(AVPacket &pkt) {
    maMutex->lock();
...
void XVPlayer::clearAudioPktList() {
    maMutex->lock();
